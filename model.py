...
class PaDiMPlus(nn.Layer):
    def __init__(self, arch='resnet18', pretrained=True, k=100, method = 'sample'):
        ...
        # self.memory_bank = self.create_parameter([21401, 384])  # bottle parameter 会参与模型更新
        
        # memory_bank = self.create_tensor(dtype='float32', name="memory_bank")  # for train
        memory_bank = paddle.zeros([21401, 384], dtype='float32')  # for model_gen and infer, [21401, 384] is bottle model shape
        self.register_buffer("memory_bank", memory_bank, persistable=True)
...
# class nearestNeighbors(nn.Layer):
#     def __init__(self):
#         super(nearestNeighbors, self).__init__()
        
#         self.memory_bank = self.create_parameter([21401, 384])  # bottle parameter 会参与模型更新
#         # self.memory_bank = self.create_tensor(name="memory_bank")
    
#     @paddle.no_grad()
#     def forward(self, embedding, n_neighbors: int = 9):
...
class PatchCore(PaDiMPlus):
    ...
    # @paddle.no_grad()中的数据不需要计算梯度，也不会进行反向传播
    @paddle.no_grad()
    ...
    # for model gen
    def forward(self, x1):
        x1 = self.forward_train(x1)
        x1 = self.generate_scores_map_infer(x1, (256, 256))
        return x1
    ...
    def compute_stats(self, embedding):
        ...
        coreset = sampler.sample_coreset()
        paddle.assign(coreset, self.memory_bank)
        print(f"Assigning the coreset as the memory bank with shape {coreset.shape}.")  # 18032,384
    ...
    # Rane optiminer
    def generate_scores_map_infer(self, embedding, out_shape, gaussian_blur = True):
            #Nearest Neighbours distances
        B, C, H, W = embedding.shape
        embedding = embedding.transpose((0,2,3,1)).reshape((B, H*W, C))
        ...
        # image_score = []
        # for i in range(B):
        #     image_score.append(self.compute_image_anomaly_score_infer(distances[:,i,:]))
        # image_score = paddle.concat(image_score, 0)
        
        image_score = self.compute_image_anomaly_score_infer(distances[:,0,:])
    ...
    def nearest_neighbors(self, embedding, n_neighbors: int = 9):
        ...
        """
        B, HW, C = embedding.shape  # [1, 1024, 384]
        # n_coreset = self.memory_bank.shape[0]
        ...
        # for i in range(B):
        #     # distances.append(cdist(embedding[i,:,:], self.memory_bank, p=2.0))  # euclidean norm
        #     distances.append(self.model_cdist(embedding[i,:,:], self.memory_bank, p=2.0))  # euclidean norm
        # distances = paddle.stack(distances, 0)  # [1, 1024, 21401]
        
        # infer one batch
        distances = self.model_cdist(embedding[0,:,:], self.memory_bank, p=2.0)  # euclidean norm
        distances = paddle.stack([distances], 0)  # [1, 1024, 21401]
        
        
        # test sdist fun
        # distances = paddle.concat([self.memory_bank, self.memory_bank, self.memory_bank], 1)
        # distances = distances[:, :1024]
        # distances = distances.transpose((1, 0))
        # distances = paddle.concat([embedding[0], distances], 1)
        # distances = distances[:, :21401]
        # distances = paddle.stack([distances], 0) 
    ...
    @staticmethod
    def compute_image_anomaly_score_infer(distance):
        """Compute Image-Level Anomaly Score for one nearest_neighbor distance map.
        """
        #distances[n_neighbors, B, HW]
        max_scores = paddle.argmax(distance[0,:])
