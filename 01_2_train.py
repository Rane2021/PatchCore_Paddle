...
def parse_args():
    ...
    parser.add_argument('--data_path', type=str, default='/media/tianru/Rane/CODE/04_huagong_proj/PatchCore_Dataset')
...
def train(args, model, train_dataloader, class_name):
    ...
    if args.inc:  # not run
        ...
        if args.einsum:
            ...
            model.compute_stats(outs)  # compute: mem_ban
        del outs
    
    # Rane: test model infer forward
    # out = model(test_infer_input)
    ...
    if args.save_model:
        ...
        # save all infer model
        save_name = os.path.join(args.save_path+'/infer_model', '{}.pdparams'.format(class_name))
        dir_name = os.path.dirname(save_name)
        os.makedirs(dir_name, exist_ok=True)
        state_dict = {
